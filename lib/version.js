...
class VersionUtils {
  ...
  static checkForGitIfNeeded(options, cwd) {
    if (VersionUtils.hasUseGit(options)) {
      return GitUtils.hasGitInstalled()
        .then((state) => {
          if (!state) {
  ...
  static createBranchGitIfNeeded(packageJsonVersion, options, cwd) {
    if (VersionUtils.hashCreateBranchGit(options)) {
      return GitUtils.createBranch(
        packageJsonVersion,
        options['git-branch-message']
          ? options['git-branch-message']
  ...
  static createTagGitIfNeeded(packageJsonVersion, options, cwd) {
    if (VersionUtils.hashCreateTagGit(options)) {
      return GitUtils.createTag(
        packageJsonVersion,
        options['git-tag-message']
          ? options['git-tag-message']
    ...
    if (!options || options.help) {
      ...
      if (VersionUtils.hasFoundPackageJsonFile(cwd)) {
        ...
        if (options.unpreid) {
          ...
          packageJsonVersion =
            VersionUtils.unpreidPackageVersion(packageJsonVersion);
        } else {
        ...
        if (options['read-only']) {
          ...
          return Promise.resolve()
            ...
            .then(() =>
              VersionUtils.doPrenpmVersionRunScriptIfNeeded(packageJson, cwd),
            )
            .then(() =>
              VersionUtils.updatePackageVersion(packageJsonVersion, cwd),
            )
            .then(() =>
              VersionUtils.updateJsonFilesIfNeeded(
                options,
                packageJsonVersion,
                cwd,
              ),
            ...
            .then(() =>
              VersionUtils.doPostnpmVersionRunScriptIfNeeded(packageJson, cwd),
            )
            .then(() =>
              VersionUtils.createCommitGitIfNeeded(
                packageJsonVersion,
                options,
                cwd,
              ),
            ...
            .then(() =>
              VersionUtils.createBranchGitIfNeeded(
                packageJsonVersion,
                options,
                cwd,
              ),
            ...
            .then(() =>
              VersionUtils.createTagGitIfNeeded(
                packageJsonVersion,
                options,
                cwd,
              ),
            ...
            .then(() =>
              VersionUtils.doPushGitIfNeeded(packageJsonVersion, options, cwd),
            )
  ...
  static doPostnpmVersionRunScriptIfNeeded(packageJson, cwd) {
    if (
      ...
      !VersionUtils.isNpmversionRunScriptDetectedInPackageJson(packageJson)
    ) {
  ...
  static doPrenpmVersionRunScriptIfNeeded(packageJson, cwd) {
    if (
      VersionUtils.isPrenpmversionRunScriptDetectedInPackageJson(packageJson) &&
      !VersionUtils.isNpmversionRunScriptDetectedInPackageJson(packageJson)
    ) {
      // if "npmversion" is into the run scripts, npm will automatically dears with the pre / post (if the command is run with "nom run npmversion")
   ...
   * @returns {Promise}
   */
  static doPushGitIfNeeded(packageVersion, options, cwd) {
    if (VersionUtils.hashPushCommitsGit(options)) {
      let remoteOriginNamePromise = VersionUtils.hasRemoteNameGit(options)
        ? Promise.resolve(options['git-remote-name'])
        : GitUtils.getRemoteName(cwd);

      return remoteOriginNamePromise.then((remoteName) => {
        let isUpStreamPromise;
        let branchName = null;

        if (VersionUtils.hashCreateBranchGit(options)) {
          branchName = GitUtils.createBranchLabel(
            packageVersion,
            options['git-branch-message']
              ? options['git-branch-message']
              : messages.GIT_BRANCH_MESSAGE,
          );
          isUpStreamPromise = GitUtils.isBranchUpstream(
            branchName,
            remoteName,
            cwd,
          );
        } else {
          isUpStreamPromise = GitUtils.isCurrentBranchUpstream(remoteName, cwd);
        }

        return isUpStreamPromise
          .then((isUpstream) => {
            if (!isUpstream) {
              if (branchName) {
              ...
              return GitUtils.getBranchName(cwd).then((retrievedBranchName) =>
                GitUtils.upstreamBranch(remoteName, retrievedBranchName, cwd),
              );
            }
          ...
          .then(() =>
            GitUtils.push(VersionUtils.hashCreateTagGit(options), cwd),
          );
      });
    }
  ...
  static getCurrentPackageJson(cwd) {
    let packageJsonContent = fs.readFileSync(
      path.resolve(path.join(cwd ? cwd : process.cwd(), PACKAGE_JSON_FILENAME)),
    );
  ...
  static hasUseGit(options) {
    ...
    return (
      VersionUtils.hashCreateCommitGit(options) ||
      VersionUtils.hashCreateTagGit(options) ||
      VersionUtils.hashPushCommitsGit(options)
    );
  }
  ...
  static isNpmversionRunScriptDetectedInPackageJson(packageJson) {
    return !!(
      packageJson &&
      packageJson.scripts &&
      packageJson.scripts.npmversion
    );
  }
  ...
  static isPostnpmversionRunScriptDetectedInPackageJson(packageJson) {
    return !!(
      packageJson &&
      packageJson.scripts &&
      packageJson.scripts.postnpmversion
    );
  }
  ...
  static isPrenpmversionRunScriptDetectedInPackageJson(packageJson) {
    return !!(
      packageJson &&
      packageJson.scripts &&
      packageJson.scripts.prenpmversion
    );
  }
  ...
  static runScriptPrenpmversion(cwd) {
    return Utils.promisedSpawn('npm', ['run', 'prenpmversion'], false, cwd);
  }
  ...
  static runScriptPostnpmversion(cwd) {
    return Utils.promisedSpawn('npm', ['run', 'postnpmversion'], false, cwd);
  }
  ...
  static updateJsonFile(options, packageVersion, jsonFilePathOrEntry, cwd) {
    ...
    let jsonFilePath = isJsonFileEntry
      ? jsonFilePathOrEntry.file
      : jsonFilePathOrEntry;
    let property = isJsonFileEntry ? jsonFilePathOrEntry.property : null;
    let filePath = path.resolve(
      path.join(cwd ? cwd : process.cwd(), jsonFilePath),
    );

    return Utils.readFile(filePath)
      .then((jsonContent) => {
        if (property) {
          return Utils.replaceJsonProperty(
            jsonContent,
            property,
            packageVersion,
          );
  ...
  static updateJsonFilesIfNeeded(options, packageVersion, cwd) {
    if (options && options.jsonFiles && options.jsonFiles.length > 0) {
      return Promises.sequential((index) => {
        if (index >= options.jsonFiles.length) {
          return null;
        }
 ...
 */
Object.defineProperty(VersionUtils, 'ERRORS', {
  writable: false,
  value: ERRORS,
});
