...
class VersionUtils {
            ...
            if (VersionUtils.hasFoundPackageJsonFile(cwd)) {
                ...
                if (options['read-only']) {
                    ...
                    return Promise
                        ...
                        .then(() => VersionUtils.doPushGitIfNeeded(packageJsonVersion, options, cwd))
    ...
    static doPushGitIfNeeded(packageVersion, options, cwd) {
        if (VersionUtils.hashPushCommitsGit(options)) {
            let remoteOriginNamePromise = VersionUtils.hasRemoteNameGit(options) ? Promise.resolve(options['git-remote-name']) : GitUtils.getRemoteName(cwd);

            return remoteOriginNamePromise
                .then(remoteName => {
                    let isUpStreamPromise;
                    let branchName = null;

                    if (VersionUtils.hashCreateBranchGit(options)) {
                        branchName = GitUtils.createBranchLabel(packageVersion, options['git-branch-message'] ? options['git-branch-message'] : messages.GIT_BRANCH_MESSAGE);
                        isUpStreamPromise = GitUtils.isBranchUpstream(branchName, remoteName, cwd);
                    ...
                    return isUpStreamPromise
                        .then(isUpstream => {
                            if (!isUpstream) {
                                ...
                                return GitUtils
                                    .getBranchName(cwd)
                                    .then(retrievedBranchName => GitUtils.upstreamBranch(remoteName, retrievedBranchName, cwd));
                            }
    ...
    static runScriptPrenpmversion(cwd) {
        return Utils
            .promisedSpawn('npm', ['run', 'prenpmversion'], false, cwd);
    }
    ...
    static runScriptPostnpmversion(cwd) {
        return Utils
            .promisedSpawn('npm', ['run', 'postnpmversion'], false, cwd);
    }
    ...
    static updatePackageVersion(packageVersion, cwd) {
        return Utils
            .promisedSpawn('npm', ['--no-git-tag-version', '--allow-same-version', 'version', packageVersion], false, cwd)
            .then(function () {
                return packageVersion;
            });
    }
    ...
    static updateJsonFilesIfNeeded(options, packageVersion, cwd) {
        if (options && options.jsonFiles && options.jsonFiles.length > 0) {
            return Promises.sequential(index => {
                ...
                return VersionUtils.updateJsonFile(options, packageVersion, options.jsonFiles[index], cwd);
            });
        }
