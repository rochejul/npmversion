        ...
        if (VersionUtils.hashPushCommitsGit(options)) {
            ...
            return remoteOriginNamePromise
                .then(remoteName => {
                    ...
                    if (VersionUtils.hashCreateBranchGit(options)) {
                        let branchName = options['git-branch-message'] ? options['git-branch-message'] : messages.GIT_BRANCH_MESSAGE;
                    ...
                    return isUpStreamPromise
                        .then(isUpstream => {
                            if (!isUpstream) {
                                return GitUtils
                                    .getBranchName(cwd)
                                    .then(branchName => GitUtils.upstreamBranch(remoteName, branchName, cwd));
                            }
