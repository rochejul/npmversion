...
class VersionUtils {
    ...
    static doPushGitIfNeeded(options, cwd) {
        if (VersionUtils.hashPushCommitsGit(options)) {
            let hasRemoteNameGit = VersionUtils.hasRemoteNameGit(options);
            let isUpstramPromise;
            ...
            return isUpstramPromise
                .then(isUpstream => {
                    if (!isUpstream) {
                        return Promise
                            .all([
                                hasRemoteNameGit ? options['git-remote-name'] : GitUtils.getRemoteName(cwd),
