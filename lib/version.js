...
class VersionUtils {
     ...
     * @returns {Promise}
     */
    static checkForGitIfNeeded(options) {
     ...
     * @returns {Promise}
     */
    static updateJsonFile(packageVersion, jsonFilePath) {
        let filePath = path.resolve(path.join(process.cwd(), jsonFilePath));

        return Utils
            .readFile(filePath)
            .then(jsonContent => Utils.replaceJsonVersionProperty(jsonContent, packageVersion))
            .then(newJsonContent => Utils.writeFile(filePath, newJsonContent))
            .then(() => GitUtils.addFile(jsonFilePath));
    }
     ...
     * @returns {Promise}
     */
    static updateJsonFilesIfNeeded(options, packageVersion) {
        if (options && options.jsonFiles && options.jsonFiles.length > 0) {
            return Promise.all(
                options.jsonFiles.map(jsonFilePath => VersionUtils.updateJsonFile(packageVersion, jsonFilePath))
            );
        }
